...
#SBATCH -J AlignCall_P2
#SBATCH --error=/home4/eschwar3/CO_NCO/1.scripts/1.logs/AlignCall_P2_%J.err
#SBATCH --output=/home4/eschwar3/CO_NCO/1.scripts/1.logs/AlignCall_P2_%J.out
...
N=1
SAMPLEFILE=/home4/eschwar3/CO_NCO/1.scripts/2.aux/uva_PE_parent2.txt
SAMPLE=$(awk 'NR ==  '$N' {print}' $SAMPLEFILE) # Passed sample prefix (ex: Sample-01)
DIR=/home4/eschwar3/CO_NCO
WORKDIR=/home4/eschwar3/CO_NCO/3.output/1.align_varcall/2.parent2 # Where files will be created
...
(>&2 echo ***BWA - mem -R***)
bwa mem ${REF} ${SEQDIR}/${SAMPLE}.1_1.fastq.gz ${SEQDIR}/${SAMPLE}.1_2.fastq.gz > ${SAMPLE}_R1R2.sam
...
gatk HaplotypeCaller \
        -R ${REF} \
        -I ${WORKDIR}/${SAMPLE}_comb_R1R2.RG.MD.sort.bam \
        -ERC GVCF \
